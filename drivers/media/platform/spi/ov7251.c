...
static int ov7251_sensor_power(struct v4l2_subdev *sd, int on)
{
	...
	if (on) {
		...
		dev_info(&pdata->pdev->dev, "Camera power on!\n");
	} else if (!on) {
		...
		dev_info(&pdata->pdev->dev, "Camera power off!\n");
	}
...
static int ov7251_stream_on(struct ov7251_state *pdata)
{
	unsigned int grf_val;

	mutex_lock(&pdata->lock);
	pdata->grf_gpio2b_iomux = ioremap((resource_size_t)
					  (GRF_BASE_ADDR + GRF_GPIO2B_IOMUX),
					  4);
	...
	dev_info(&pdata->pdev->dev, "Camera stream_on!\n");
	mutex_unlock(&pdata->lock);
	return 0;
}

static int ov7251_stream_off(struct ov7251_state *pdata)
{
	...
	dev_info(&pdata->pdev->dev, "Camera stream_off!\n");
...
static int ov7251_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	if (enable)
		return ov7251_stream_on(pdata);
	else
		return ov7251_stream_off(pdata);
}

static const struct v4l2_subdev_video_ops ov7251_subdev_video_ops = {
