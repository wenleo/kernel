...
void rk1608_hw_init(struct spi_device *spi)
{
	...
	spi2apb_safe_w32(spi, 0x12030040, 0xf000a000);
	spi2apb_safe_w32(spi, 0x12030044, 0x000f000a);
}
		 ...
		 s32 addr, s32 *data, size_t data_len)
{
	...
	s32 read_cmd = APB_CMD_READ | (real_len << 14 & 0xffff0000);
...
static int rk1608_stream_on(struct rk1608_state *pdata)
{
	...
	if (pdata->cam_sd->ops->video->s_stream(pdata->cam_sd, 1))
		dev_err(pdata->dev, "Call the sensor func failed!");
	else
		dev_info(pdata->dev, "Call the sensor func success!");
...
static const struct v4l2_subdev_core_ops rk1608_core_ops = {
	...
	.g_register	= rk1608_sensor_get_register,
	.s_register	= rk1608_sensor_set_register,
#endif
};
...
static int rk1608_parse_dt_property(struct rk1608_state *pdata)
{
	...
	ret = of_property_read_u32(node, "default_voltage",
				   &pdata->default_voltage);
	if (ret <= 0)
		dev_warn(dev, "can not get default_voltage!");
...
static int rk1608_probe(struct spi_device *spi)
{
	...
	if (ret)
		dev_err(rk1608->dev, "get remote node dev err(%d)\n", ret);
	rk1608->sensor_cnt = 0;
