...
mpp_dma_get_buffer(struct mpp_dma_session *session, int fd)
{
...
static void mpp_dma_buffer_release(struct kref *ref)
{
...
int mpp_dma_release_fd(struct mpp_dma_session *session, int fd)
{
	...
	buffer = mpp_dma_get_buffer(session, fd);
	...
	kref_put(&buffer->ref, mpp_dma_buffer_release);
...
dma_addr_t mpp_dma_import_fd(struct mpp_dma_session *session, int fd)
{
	...
	if (session->buffer_nums >= BUFFER_LIST_MAX_NUMS - 1) {
		struct mpp_dma_buffer *loop_buffer = NULL, *n;
		/*
		...
		list_for_each_entry_safe(loop_buffer, n,
			...
			kref_put(&loop_buffer->ref, mpp_dma_buffer_release);
		}
...
struct mpp_iommu_info *mpp_iommu_probe(struct device *dev)
{
	 ...
	 */
	common_iommu_setup_dma_ops(dev, 0x0, dev->coherent_dma_mask + 1,
				   info->domain->ops);
